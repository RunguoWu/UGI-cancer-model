...
d4s <- readRDS(file.path("/data/WIPH-CanDetect/HealthEco/work_data", paste0("study_pop_", pop_nam, ".rds")))

d4s_no_red <- d4s %>%
  filter(ng12_red_flag == "No red flag")

d4s_surv <- readRDS(file.path("/data/WIPH-CanDetect/HealthEco/work_data", paste0("study_pop_", pop_nam, ".rds"))) %>% 
...
sx_mod_list <- lapply(site_name, function(st) {
  ...
  stpm2(fm, data = d4s_sub, df = 5)
})

names(sx_mod_list) <- site_name
...
params_list <- readRDS(file.path("/data/WIPH-CanDetect/HealthEco/work_data", "params_list_1yrIndex_upd2026.rds"))
...
write.csv(rt, file.path(output, "MV_stagedist_5yrsurv_1yrIndex_2026-09.csv"))

rm(ind_output_list_img, ind_output_list_2ww, tx_list,
...
# rm(dt_list)
# 
# saveRDS(params_all, file.path(output, paste0("params_all_for_bootstrap_1yrIndex.rds")))

params_all <- readRDS(file.path(output, paste0("params_all_for_bootstrap_1yrIndex.rds")))
...
saveRDS(psa_master, file.path(output, "psa_results_upd2026_1yrIndex.rds"))
...
write.csv(panc_stage, file.path(output, "MV_stagedist_panc_stage_1yrIndex_2026-09.csv"))
...
write.csv(oeso_stage, file.path(output, "MV_stagedist_oeso_stage_1yrIndex_2026-09.csv"))
...
write.csv(stom_stage, file.path(output, "MV_stagedist_stom_stage_1yrIndex_2026-09.csv"))
...
write.csv(galb_stage, file.path(output, "MV_stagedist_galb_stage_1yrIndex_2026-09.csv"))
...
write.csv(five_year, file.path(output, "MV_five_year_1yrIndex_2026-09.csv"))