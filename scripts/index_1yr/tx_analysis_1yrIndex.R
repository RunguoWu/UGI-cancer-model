...
# saveRDS(tx_list, file.path(wd, "tx_list_1yrIndex.rds"))
saveRDS(tx_list, file.path(wd, "tx_list_1yrIndex_upd2026.rds"))
...
saveRDS(tx_list, file.path(wd, "tx_list_1yrIndex_upd2026_bySiteOnly.rds"))
...
# write.csv(table_age0, file.path(output, "aft_wei_age70plus0_1yrIndex.csv"), row.names = FALSE)
# write.csv(table_age1, file.path(output, "aft_wei_age70plus1_1yrIndex.csv"), row.names = FALSE)

write.csv(table_age0, file.path(output, "aft_wei_age70plus0_1yrIndex_upd2026.csv"), row.names = FALSE)
write.csv(table_age1, file.path(output, "aft_wei_age70plus1_1yrIndex_upd2026.csv"), row.names = FALSE)

# Compare 1yr and 2yr population ------------------------------------------
rm(list = ls())
library(tidyverse)
library(survival)
library(gtsummary)
...
dt_1yr <- readRDS(file.path(wd, paste0("study_pop_", pop_nam, ".rds")))

pop_nam <- "symp_stageImputed"
dt_2yr <- readRDS(file.path(wd, paste0("study_pop_", pop_nam, ".rds")))

# those lost from changing retrospective window from 2yr to 1yr
d4s <- dt_2yr %>% filter(!e_patid %in% dt_1yr$e_patid)
...
rt <- d4s %>% select(age_index, gender, ethn2_mis, imd5_origin, site, stage,
  ...
  add_n() %>%
  as_flex_table() %>%
  # flextable::save_as_docx(path = file.path(output, paste0("table1_popLostin1yr.docx")))
  flextable::save_as_docx(path = file.path(output, paste0("table1_popLostin1yr_upd2026.docx")))

# those who stay after the change
d4s <- dt_2yr %>% filter(e_patid %in% dt_1yr$e_patid)
...
rt <- d4s %>% select(age_index, gender, ethn2_mis, imd5_origin, site, stage,
  ...
  add_n() %>%
  as_flex_table() %>%
  # flextable::save_as_docx(path = file.path(output, paste0("table1_popNOTLostin1yr.docx")))
  flextable::save_as_docx(path = file.path(output, paste0("table1_popNOTLostin1yr_upd2026.docx")))
